\documentclass[../main.tex]{subfiles}

\begin{document}

\index{Players ! Eliskases, Erich Gottlieb $\square$}
\index{Players ! Capablanca, Jose Raul $\blacksquare$}
\subsection{Eliskases, Erich Gottlieb vs. Capablanca, Jose Raul -- 1-0 -- D17}
\begin{multicols*}{2}

% Game's Opening ECO data
\index{Openings ! D17 -- Slav: Central, Carlsbad, Main Line}
\subsubsection{D17 -- Slav: Central, Carlsbad, Main Line}
\begin{flushleft}
\newchessgame[id=eco]
\longmoves
\mainline{1. d4 d5 2. c4 c6 3. Nf3 Nf6 4. Nc3 dxc4 5. a4 Bf5 6. Ne5 Nbd7 7. Nxc4 Qc7 8. g3 e5 9. dxe5 Nxe5 10. Bf4 Nfd7 11. Bg2 f6}
\begin{center}
\begin{tabular}{C{60mm}}
\chessboard[setfen={r3kb1r/ppqn2pp/2p2p2/4nb2/P1N2B2/2N3P1/1P2PPBP/R2QK2R w KQkq - 0 12},
             pgfstyle=border,
             color=YellowGreen,
             markfields={f7,f6},
             pgfstyle=straightmove,
             color=YellowGreen,
             markmoves={f7-f6}]
\newline
\xskakset{moveid=\xskakgetgame{lastmoveid}}
\printmovercolor{\xskakgetgame{lastplayer}}
\end{tabular}
\end{center}
% PGN tags
\begin{tabular}{L{60mm} R{10mm}}
\multicolumn{2}{l}{\textbf{Semmering, Semmering/Baden}}\\ 
\multicolumn{2}{l}{1937 Round 10}\\[3mm]
\textbf{$\square$ \hspace{2mm} Eliskases, Erich Gottlieb} & \textbf{1}\\ 
\textbf{$\blacksquare$ \hspace{2mm} Capablanca, Jose Raul} & \textbf{0}\\ 
\end{tabular}

% Game's final diagram and result
\newchessgame[id=overview]
\longmoves
\hidemoves{1. d4 d5 2. c4 c6 3. Nf3 Nf6 4. Nc3 dxc4 5. a4 Bf5 6. Ne5 Nbd7 7. Nxc4 Qc7 8. g3 e5 9. dxe5 Nxe5 10. Bf4 Nfd7 11. Bg2 f6}\mainline{12. O-O Rd8 13. Qc1 Be6 14. Nxe5 Nxe5 15. a5 a6 16. Ne4 Bb4 17. Bd2 Qe7 18. Bxb4 Qxb4 19. Qc5 Qxc5 20. Nxc5 Bc8 21. Rfd1 Ke7 22. b3 Nf7 23. e4 Rd6 24. Rxd6 Kxd6 25. b4 Kc7 26. Rd1 Rd8 27. Rxd8 Nxd8 28. f4 b6 29. axb6+ Kxb6 30. Bf1 Ne6 31. Na4+ Kc7 32. Kf2 g5 33. Ke3 gxf4+ 34. gxf4 Ng7 35. Nc5 Ne6 36. Nxe6+ Bxe6 37. Kd4 Kb6 38. Bc4 Bg4 39. e5 fxe5+ 40. fxe5 h6 41. h4 Bh5 42. e6 Be8 43. Bd3 Kc7 44. Kc5 Bh5 45. Bh7 Bg4 46. e7 Kd7 47. Be4 Kxe7 48. Bxc6 Be2 49. Bb7 Kd7 50. Kb6 Kd6 51. Bxa6 Bf3 52. Ka5 Bc6 53. Bb5 Bf3 54. Bd3 Bc6 55. Bc2 Kc7 56. Ba4 Bf3 57. b5 Kb7 58. b6 Be2 59. Bc2 Bf3 60. Bd3 Bg2 61. Ba6+ Kc6 62. Bc8 Bf1 63. Bg4 Bd3 64. Bf3+ Kd6 65. Bb7 Be2 66. Ba6 Bf3 67. Bf1 Bb7 68. Bh3 Ke7 69. Kb5 Kd6 70. Bg4 Ke7 71. Kc5 Bg2 72. Bc8 Kd8 73. Ba6 Bf3 74. Kd6 Bg2 75. Bc4 Kc8 76. Bd5 Bf1 77. Ke6 Be2 78. Kf6 Kd7 79. Kg6 h5 80. Kg5 Kd6 81. Bf7 Kc6 82. Bxh5 }

\begin{center}
\begin{tabular}{C{60mm}}
\xskakset{moveid=\xskakgetgame{lastmoveid}}
\chessboard[setfen=\xskakget{nextfen},
       pgfstyle=border,
       color=YellowGreen,
       markfields={f7,h5},
             pgfstyle=straightmove,
             color=YellowGreen,
             markmoves={f7-h5}]
\newline
82.\,\xskakget{lan} ...
\newline
1-0
\end{tabular}
\end{center}
\end{flushleft}
\end{multicols*}
\newpage
\end{document}
